...
# Common base model for the party resoruce
...
# Form modle for the Party resource
...
class GiftBase(SQLModel):
    ...
    price: Decimal = Field(
        decimal_places=2, ge=0
    )  # Decimal constraints enforced by pydantic
    link: Optional[str]
    party_id: UUID = Field(
...
class GuestBase(SQLModel):
    ...
    party_id: UUID = Field(
        default=None, foreign_key="party.uuid"
    )  # Defines a foreign key connecting the guest to a party


# Database model for the Guest resource
class Guest(GuestBase, table=True):
    uuid: UUID = Field(default_factory=uuid4, primary_key=True)
    party: Party = Relationship(back_populates="guests")  # Establishes ORM relationship


# Form model for guest resource
